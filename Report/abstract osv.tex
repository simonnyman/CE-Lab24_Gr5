...
\begin{abstract}
    ...
    The main focus of ours was the development of the natigational software, which will be be explained in depth witht the use of figures and the like, all aiming to clarify and explain the thought process behind the algorithm as clearly as possible.
...
All the software will be implemented in python using the Robot Operating Software framework (ROS). For the sensors we will be using a LiDAR sensor capable of measuring distances in a 360 degree view, and an RGB-sensor which differentiates between red, green and blue colors. 
...
and a maximum rotational velocity of 2.84 rad/s. In order keep the robot as agile as possible with no wires attached, we used an 800 mA Li-Po battery and established a wireless connection, which will be specified further in a later section.
...
This was done by simply running a simple navigational algorithm on the robot several times until we figured out that they were organized in a way such that 0$^\circ$ was at the back of the robot, and then continued clockwise around the robot.
...
As the navigation is based around the concept of avoiding for objects obstructing the path of the robot, we "ignored" the 180 deegrees behind the robot
We "ignored" the backwards facing 180 deegrees, by making the robot move forward in full speed.
The same concept is applied to two remaining 15 deegree cones of the forward facing 180 deegrees, which enables the robot to run parralel to an object without turning away.
\begin{figure}[htbp]
...
With this in mind, we placed the RGB sensor as close to the ground as the robot allowed for, in order to avoid as much light disturbance as possible.
...
The RGB was connected to the Raspberry Pi using four pins, two of these being the SDA (Serial Data Line) and SCL (Serial Clock Line)\cite{b4}. The SDA is the pin resposible for transfering raw data from the sensor to the Pi, where it is interpreted and processed in order to extract the red, green, and blue values.
...
If all these factors were present, an LED would light up and our \textit{victim counter} variable would be incremented by 1.
As the implementation was finished, a short delay was added to the detection funciton as the sensor would otherwise be able to detect multiple victims from a single red spot, courtesy of the high clock speed.
...
The same thing was done for our \textit{collision counter} variable, as this was also signified by an LED, only in a different color.
...
Whenever a new step had to be taken, we discussed it between the two of us and came up with a rough idea of how to approach the problem and would afterwards work together on the solution, whether it was our navigation software or the integration of a new sensor or other external part.
During the writing of the report, the workload was divided in the follwing way:
