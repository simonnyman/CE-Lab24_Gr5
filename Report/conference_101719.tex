...
The testing and experiments consisted mostly of figuring out how we wanted the robot to respond to a given scenario, and then creating that scenario and evaluting what actually happend.
...
As mentioned in \textit{Navigation} the main parameters our overall strategy allowed us to adjust, was how we would divide the LiDAR readings into different angle and distane intervals, and then what linear and angular speed the robot should move with.
...
For for the speeds of our robot we found the maximum linear speed to be 0.21 meters per second, and the maximum angular speed to be 2.50 radians per second.
...
We found that before the robot had "registered" the obstacle (see), to then process the information (think) and act, the robot would have moved into a diffrent distance zone, often time leading to a collision.
...
What we continued to discover throughout the testing and experimentation, was that often times simplicity was the way to go.
Making the different components more complex led to mostly similiar and sometimes even worse results.
That being said of course there is probably a way to get better results than ours, as we after all are beginners.
That would probably require a complely different concept from the top and down.
We are positive that a lot more complex solution is required to acheive a better performance, but compared to our solution those performance gains would be marginal relative to the complexity needed  
\section{Conclusion}
